...
### 3.4 VIP 等級與結構化標籤（TAG_CATEGORIES）相容性規則

`TAG_CATEGORIES` 中各標籤對不同 VIP 等級有嚴格的相容性限制，系統應在分配標籤時自動過濾不相容組合。

#### 3.4.1 禁止清單（Blacklist）

| 標籤 | 分類 | VVVIP | VVIP | VIP | Normal |
|------|------|:-----:|:----:|:---:|:------:|
| 低收益客戶 | 客戶屬性 | ❌ 禁止 | ❌ 禁止 | ❌ 禁止 | ✅ 可用 |
| 頻繁預借現金 | 交易互動 | ❌ 禁止 | ❌ 禁止 | ⚠️ 謹慎 | ✅ 可用 |
| 近期首次產生循環利息 | 交易互動 | ❌ 禁止 | ❌ 禁止 | ⚠️ 謹慎 | ✅ 可用 |
| 長期信卡循環利息 | 交易互動 | ❌ 禁止 | ❌ 禁止 | ❌ 禁止 | ✅ 可用 |
| 刷卡經常額度不足 | 產品屬性 | ❌ 禁止 | ❌ 禁止 | ⚠️ 謹慎 | ✅ 可用 |
| 經常在信卡額度上限邊緣 | 產品屬性 | ❌ 禁止 | ❌ 禁止 | ⚠️ 謹慎 | ✅ 可用 |
| 信卡臨時額度上調 | 產品屬性 | ❌ 禁止 | ❌ 禁止 | ⚠️ 謹慎 | ✅ 可用 |
| 頻繁聯絡客服 | 客戶屬性 | ❌ 禁止 | ⚠️ 謹慎 | ✅ 可用 | ✅ 可用 |

> ❌ 禁止：系統自動過濾，不得分配  
> ⚠️ 謹慎：非主動分配，僅在有明確業務事件時才可設定

#### 3.4.2 優先標籤（Preferred Tags）

高端 VIP 客戶在隨機挑選標籤後，若無任何「優先標籤」，系統應自動補充一個：

| VIP 等級 | 優先標籤（至少應出現其中一個）|
|----------|----------------------------|
...
1. 從各 TAG_CATEGORIES 隨機挑選標籤（以客戶 ID seed 決定）
2. 濾除該 VIP 等級的禁止標籤（Blacklist）
3. 若高端 VIP 客戶無任何優先標籤（Preferred Tags），補充一個
4. 合併 persona 特定標籤（personaTagMap）
5. 最終去重
```

**常見陷阱**：
- ❌ VVVIP 被隨機分配到「低收益客戶」→ 應由系統黑名單自動過濾
- ❌ VVIP 客戶標籤全為負向交易行為（如循環利息）→ 應補充投資類優先標籤
- ❌ VIP 客戶有「長期信卡循環利息」→ 與其 riskScore A/A+ 的信用等級矛盾
...
- [ ] **結構化標籤 Blacklist 合規**：`低收益客戶`、`長期信卡循環利息`、`刷卡經常額度不足` 等標籤不應出現於 VVVIP/VVIP；VIP 不得含 `長期信卡循環利息`（詳見 3.4.1）
- [ ] **高端 VIP 優先標籤存在**：VVVIP/VVIP 客戶的結構化標籤中，至少應含一個投資/高資產相關標籤（詳見 3.4.2）
...
const validateCustomerData = (customer) => {
  ...
  // 結構化標籤與 VIP 等級相容性（參見 3.4）
  const VIP_TAG_BLACKLIST = {
    VVVIP: ["低收益客戶", "頻繁預借現金", "近期首次產生循環利息", "長期信卡循環利息", "刷卡經常額度不足", "經常在信卡額度上限邊緣", "信卡臨時額度上調"],
    VVIP:  ["低收益客戶", "頻繁預借現金", "近期首次產生循環利息", "長期信卡循環利息", "刷卡經常額度不足", "信卡臨時額度上調"],
    VIP:   ["低收益客戶", "長期信卡循環利息"],
  };
  const blacklist = VIP_TAG_BLACKLIST[customer.vipLevel] || [];
  const allTags = [
    ...(customer.tags || []),
    ...((customer.structuredTags || []).map(t => t.name || t)),
  ];
  blacklist.forEach((banned) => {
    if (allTags.includes(banned)) {
      issues.push(`${customer.vipLevel} 客戶不應含標籤「${banned}」`);
    }
  });
...
| 2026-04-22 | v1.4 | 新增第 3.4 節「VIP 等級與結構化標籤相容性規則」：Blacklist、Preferred Tags、設計邏輯；同步更新一致性檢驗清單與驗證函式 |
